...
class ChatAdapter : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
  // 여러개의 xml을 사용하는 recyclerView의 adapter는 뷰홀더도 여러개가 존재하게 됩니다.RecyclerView.ViewHolder만을 제네릭에 넘겨주고, 후에 각각의 뷰홀더로 형변환을 한 후에 사용합니다. **이해가 잘 안 될 수도 있는데.. 끝까지 코드를 따라하고 난 후에는 이해가 될거에요:> !
}
```
...
override fun getItemViewType(position: Int): Int {
  ...
  // 저는 아~주 단순하게 제가 쓴 대화는 id 에 "me"라는 값을 넣었고, 상대방이 보낸 대화에는 "yours"라는 값을 넣었어요! 두개의 xml을 구분하는 기준은 마음대로 정하면 됩니다! DB에 저장된 토근값으로 비교를 한다거나.. 하면 더 좋겠즁?!
...
현재는 해당 activity를 종료하면 대화기록이 모두 지워지도록 되어있습니다. 이와 관련된 문제는, socket.io를 이용한 채팅기능을 모두 구현 한 후, `3. 내부디비를 이용하여 대화내용 저장하기` 다뤄보도록 할게요!